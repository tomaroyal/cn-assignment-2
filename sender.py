...
class Timer(object):
    ...
    def timeout(self):       
        if(not self.isrunning()):
            ...
            if(self.gettime() - self.starttime > self.duration):
                return(True)
            else:
                return(False)
...
FILE_SIZE = 20000
NO_OF_PACKETS = 20
PACKET_SIZE = FILE_SIZE/NO_OF_PACKETS
SLEEP_INTERVAL = 0.05
...
timer = Timer()
timer.setduration(TIMEOUT_INTERVAL)
...
def send_data(soc, file):
    packets[]

    for x in range(NO_OF_PACKETS):
        packets.append(f.read(PACKET_SIZE))

    if(WINDOW_SIZE > NO_OF_PACKETS):
        WINDOW_SIZE = NO_OF_PACKETS

    start_new_thread(receive, (soc,))

    while start_of_window < NO_OF_PACKETS:
        lock.acquire()

        while WINDOW_SIZE > next_frame_to_send - start_of_window:
            tosend = next_frame_to_send + packets[next_frame_to_send]
...
for entry in range(NO_OF_PACKETS):
...
# close connection
print("average=",sum/NO_OF_PACKETS)
sock.close()